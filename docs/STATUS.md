...
> 2026-07-04 기준. 이 문서 하나로 "지금 뭐가 만들어져 있고, 어떻게 돌아가고, 뭐가 남았나"를
> 파악할 수 있게 쓴 오너용 요약. 상세 문서는 맨 아래 [문서 지도](#문서-지도) 참조.

## 한 줄 요약

**필리핀 파일럿용 위치기반 단기 알바 매칭 서비스의 MVP가 웹·모바일·백엔드 전부
실서비스 상태로 배포되어 있고, Android 앱은 Play Store 심사에 제출됐다(2026-07-04).**
공고 등록 → 지원 → 매칭 → 도착 → PIN 완료 → 상호 리뷰의 전체 루프 + 취소/노쇼/분쟁/
만료/계정삭제까지 동작. 남은 건 Play 심사 통과와 Semaphore(실 SMS) 계정 승인.
...
| 모바일 앱 (Android 우선)  | Google Play (com.gigon.app, PH+KR)        | 🔜 심사 제출됨 2026-07-04 (승인 시 자동 공개) |
...
├── packages/api  → @repo/api       쓰기 API 계약 (zod 스키마 + fetch 클라이언트)
└── supabase/     → 스키마·시드 (마이그레이션 6개 — 쓰기 RPC는 drop됨)
```
...
| 장치               | 구현                                                                      |
| ------------------ | ------------------------------------------------------------------------- |
| 상태머신 우회 불가 | 모든 전이는 서버 API(app.gigon.io/api)로만 (클라이언트가 DB를 직접 못 씀) |
...
- **쓰기 API 15개** (Next.js Route Handlers, `apps/app/app/api/**` — TS 비즈니스 로직,
  drizzle+트랜잭션, 감사 로그 직접 기록. vitest 통합 테스트 31케이스):
  온보딩 3(/api/onboarding·/api/invites/redeem·/api/profile/role) ·
  거래 7(gigs·apply·select·arrive·pin·pin/verify·messages) ·
  예외 5(no-show·dispute·cancel×2·review)
- **DB에 남은 함수**: `expire_stale_gigs`(pg_cron+클라 스윕) · `handle_new_user`(트리거) —
  구 쓰기 RPC 15개는 2026-07-03 drop됨
- **RLS 방침**: 전 테이블 ON이되 row rule은 권한 레이어가 아님 — 읽기는 로그인 사용자
  전체 허용 한 줄, 쓰기는 정책 0개(API만). 비밀값은 grant 없는 테이블로 격리
- **실시간**: gigs/applications/matches/messages 변경 구독 → 토스트+리프레시

## 인증/계정

- 전화번호 + SMS OTP (Supabase Auth). 계정 1개 = 역할 전환 가능(워커↔사장님)
- 실 SMS는 **Semaphore**(필리핀 로컬) 연동 완료 — Semaphore 계정 승인·Sender Name·크레딧
  충전이 끝나면 실번호 로그인 활성화. 그 전까지는 아래 테스트 번호만 로그인 가능

| 테스트 번호   | OTP    | 용도                                             |
| ------------- | ------ | ------------------------------------------------ |
| 0917 123 4001 | 123456 | 워커 (Marites Santos) — **Play 리뷰어 계정**     |
| 0917 123 4002 | 123456 | 사장님 (Kape Lokal Annex) — **Play 리뷰어 계정** |
| 0917 123 4003 | 123456 | 여분                                             |

> ⚠️ 4001/4002는 Play Console "Sign in details"에 등록된 심사용 계정 —
> 삭제하거나 역할/OTP를 바꾸면 이후 모든 업데이트 심사가 막힌다.
...
0. ~~**백엔드 API 이관**~~ — ✅ 2026-07-03 완료. 쓰기 15개가 Route Handlers로 이관,
   구 RPC drop, Vercel 배포·프로드 스모크까지 통과.
   기록: [docs/service/api-migration.md](service/api-migration.md).
   ⚠️ 남은 스모크 1개: **Expo Go 실기기** 로그인→지원→도착→PIN (기기 필요)
1. **실 SMS 연동** — ✅ Semaphore + Send SMS Hook 연동 배포 완료 (Twilio 대비 ~1/15 비용).
   남은 건 Semaphore 계정 쪽: 승인 대기 해소 → Sender Name 신청 → 크레딧 충전
2. **시드 데모 데이터 결정** — 클라우드의 데모 가맹점 6곳/공고 6건. 자동 만료가 켜져서
   방치하면 24h 후 EXPIRED로 자연 소멸. 유지하려면 시드 갱신 필요
3. **모바일 스토어 배포** — ✅ **Play Store 심사 제출 완료(2026-07-04)**: Production
   릴리스 vc3(계정 삭제 포함), 국가 PH+KR, 전체 선언(로그인 정보·광고·등급·타겟 연령·
   Data safety 등) + 스토어 등록정보 완료. 승인되면 자동 공개(Managed publishing off).
   리뷰어용 테스트 계정 = 아래 4001(워커)/4002(사장님) — **지우면 안 됨**.
   승인 후: Maps 키 제한(App integrity의 SHA-1). 선언 상세·근거:
   [docs/daily/2026-07-04.md](daily/2026-07-04.md)
   **iOS(App Store)**: 에셋·등록정보 준비 완료(2026-07-04) — 6.9" 스크린샷 4장,
   알파 제거 아이콘, 카피·App Privacy·심사노트 일체:
   [docs/brand/assets/app-store/listing.md](brand/assets/app-store/listing.md).
   남은 건 Apple Developer Program 등록($99/년) → `eas build/submit -p ios`
4. **푸시 알림** — ✅ 구현 완료(2026-07-04): push_tokens + /api/push/*, 5개 이벤트
   (지원·매칭·채팅·취소·노쇼) 서버 발송, 모바일 토큰 등록/해제. Android 실수신은
   3번의 Firebase 연결 후 EAS 빌드에서부터 (Expo Go는 SDK 53+ 원격 푸시 불가)
...
- 알림 벨 실기능 · 프로필 편집 · My gigs/History 페이지 · Sentry
- ~~PostHog 퍼널~~ — ✅ 2026-07-04 계측 완료. 핵심 퍼널은 서버 API에서 캡처라
  웹·모바일 공통으로 잡힘. Vercel env 설정 완료, 다음 배포부터 수집.
  이벤트 스키마·퍼널 정의: [docs/daily/2026-07-04.md](daily/2026-07-04.md)
...
| DB 스키마·쓰기 API 상세·배포 런북 | [docs/service/backend.md](service/backend.md)       |
